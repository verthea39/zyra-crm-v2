...
import { updateServiceTemplateAction } from "@/app/actions/services";
import Link from "next/link";
import { ArrowLeft, Plus, Trash2 } from "lucide-react";

export function EditServiceForm({ template }: { template: any }) {
  const [state, formAction, isPending] = useActionState(updateServiceTemplateAction, undefined);
  
  const initialSteps = template.stepDefs?.length 
    ? template.stepDefs.map((step: any, index: number) => ({ id: step.id || Date.now() + index, name: step.name, description: step.description }))
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-semibold tracking-tight">Edit Service Template</h1>
      </div>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  name="basePrice"
                  defaultValue={template.basePrice?.toString() || ""}
                  className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
