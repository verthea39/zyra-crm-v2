...
import { advanceCaseStage, updateCaseGovRef } from "@/app/actions/pipeline";
...
export function KanbanBoard({ cases, onCasesChange }: { cases: any[], onCasesChange: React.Dispatch<React.SetStateAction<any[]>> }) {
  ...
  const renderMobileView = () => (
    <div className="md:hidden flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        {renderCards(cases.filter(c => c.stage === activeMobileTab), STAGES.find(s => s.id === activeMobileTab)!)}
      </div>
    </div>
  );
  ...
  const renderCards = (stageCases: any[], stage: typeof STAGES[0]) => {
      ...
      return (
        ...
        >
          ...
          <div className="flex border-t border-slate-200 bg-slate-50/50 overflow-hidden shrink-0" onClick={e => e.stopPropagation()}>
            ...
            {nextStage && (
              <button
                onClick={() => moveCase(c.id, nextStage.id)}
                disabled={!!updating}
